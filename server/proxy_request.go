...
func (s *VimeoService) proxyRequest(w http.ResponseWriter, r *http.Request) {
	...
	// Create requet object used for proxy operation
	...
	if rangeProvided {
		...
		// Attempt cache lookup, if err respBytes is empty
		// if no err respBytes holds byte array from cache
		...
		// Copy body from resp to respBytes buffer
		...
		// Write respBytes to ResponseWriter and return to client
		w.Write(respBytes)
		return
	}

	// No range provided, simply proxy origin's response back to client
	resp, err := s.httpClient.Do(req)
	defer resp.Body.Close()
	if err != nil {
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
